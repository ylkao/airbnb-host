...
function idealPrice($listings, $assoc=true) {
  ...
  $result = 0;
  if ($total == 0) {
    $result = round($sum, count($listings), 2); //prevent division by 0 
  } else {
    $result = round($sum / $total, 2);
  }
  return number_format($result, 2); //keep trailing 0s
}
...
/**
  * Calculate the optimal price to maximize bookings based on user input from index.php
*/
function main() {
  try {
